...
from siaslice import SiadSession, siad_get, siad_post
...
class GetRequest(asynctest.TestCase):
    ...
    async def setUp(self):
        self.session = SiadSession(DOMAIN, API_PASS)
    ...
    async def test_version_check(self):
        response = await siad_get(self.session, 'daemon', 'version')
        self.assertEqual(response.status, 200)
        self.assertIn('version', await response.json())

    async def test_stream_nonexistent_file(self):
        response = await siad_get(self.session, 'renter', 'stream',
...
class PostRequest(asynctest.TestCase):
    ...
    async def test_valid_siapath(self):
        response = await siad_post(self.session, b'', 'renter', 'validatesiapath',
